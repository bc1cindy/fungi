...
use fungi_transport::{
    BuildError, Channel, ChannelBuilder, Codec, CodecChannel, RecvError, RecvHalf, SendError,
    SendHalf, into_stream,
};
use fungi_transport_capnp::{CapnpBuilder, CapnpChannel, serve, serve_builder};
use fungi_transport_testkit::{
    mem::{MemChannel, MemConfig, duplex, network},
    testkit,
};
...
fn server<C>(backend: C, io: tokio::io::DuplexStream) -> std::thread::JoinHandle<()>
where
    C: Channel<Vec<u8>, SendError = SendError, RecvError = RecvError> + 'static,
{
    std::thread::spawn(move || {
        let runtime = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap();
        tokio::task::LocalSet::new().block_on(&runtime, serve(backend, io));
    })
}
fn wrap(backend: MemChannel, max: usize) -> (CapnpChannel, std::thread::JoinHandle<()>) {
...
async fn duplex_bursts_preserve_every_message_under_backpressure() {
    let (mut left, mut right, servers) = pair(MemConfig::default());
    async fn exchange(channel: &mut CapnpChannel, tag: u8) -> BTreeSet<Vec<u8>> {
...
async fn typed_channels_use_the_existing_codec_adapter() {
    ...
    let (mut left, mut right) = (
        CodecChannel::new(left, ByteCodec),
        CodecChannel::new(right, ByteCodec),
    );
    deadline(async {
        left.send(42).await.unwrap();
        assert_eq!(right.recv().await.unwrap(), 42);
        let (mut sender, mut receiver) = right.split();
...
async fn builders_reconnect_and_keep_channels_alive_after_builder_drop() {
    ...
    let first = std::thread::spawn(move || {
        ...
        tokio::task::LocalSet::new().block_on(
            &rt,
            serve_builder(connector, |_| Ok(fungi_transport_testkit::mem::MemAddr), io),
        );
    });
...
async fn subprocess_supports_concurrent_channels_and_reports_crashes() {
    ...
    let mut blocked = deadline(builder.build(&Vec::new())).await.unwrap();
    assert!(
...
#[derive(Debug)]
struct FailingReceive(MemChannel, std::sync::Arc<std::sync::atomic::AtomicUsize>);
#[derive(Debug)]
struct FailedHalf(std::sync::Arc<std::sync::atomic::AtomicUsize>);
impl RecvHalf for FailedHalf {
...
impl SendHalf for FailingReceive {
    type SendError = SendError;
    async fn send(&mut self, message: Vec<u8>) -> Result<(), SendError> {
        self.0.send(message).await
    }
}
impl RecvHalf for FailingReceive {
    type RecvError = RecvError;
    async fn recv(&mut self) -> Result<Vec<u8>, RecvError> {
        FailedHalf(self.1.clone()).recv().await
    }
}
impl Channel for FailingReceive {
    type SendHalf<'a> = <MemChannel as Channel>::SendHalf<'a>;
    type RecvHalf<'a> = FailedHalf;
    fn split(&mut self) -> (Self::SendHalf<'_>, Self::RecvHalf<'_>) {
        (self.0.split().0, FailedHalf(self.1.clone()))
    }
}
#[tokio::test(flavor = "multi_thread")]
async fn receive_backend_error_terminates_the_stream_with_diagnostics() {
    ...
    let server = server(FailingReceive(backend, calls.clone()), io);
...
impl SendHalf for TrackedChannel {
    type SendError = SendError;
    async fn send(&mut self, message: Vec<u8>) -> Result<(), SendError> {
        self.channel.send(message).await
    }
}
impl RecvHalf for TrackedChannel {
    type RecvError = RecvError;
    async fn recv(&mut self) -> Result<Vec<u8>, RecvError> {
        self.channel.recv().await
    }
}
impl Channel for TrackedChannel {
    type SendHalf<'a> = <MemChannel as Channel>::SendHalf<'a>;
    type RecvHalf<'a> = <MemChannel as Channel>::RecvHalf<'a>;
    fn split(&mut self) -> (Self::SendHalf<'_>, Self::RecvHalf<'_>) {
        self.channel.split()
    }
}
...
#[tokio::test(flavor = "multi_thread")]
async fn abandoned_builds_and_mass_drops_release_backends_while_the_builder_stays_alive() {
    ...
    let server = std::thread::spawn(move || {
        let rt = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap();
        tokio::task::LocalSet::new().block_on(&rt, serve_builder(backend, Ok, io));
    });
    let mut builder = CapnpBuilder::connect(client, MAX).unwrap();
    let input = Vec::new();
    {
